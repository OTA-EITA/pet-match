...
func (s *UserService) Create(user *models.User, passwordHash string) (*models.User, error) {
	// Store user data
	...
	// Store email -> user ID mapping
	emailKey := fmt.Sprintf("user:email:%s", user.Email)
	err = s.redisClient.Set(s.ctx, emailKey, user.ID, 0).Err()
	if err != nil {
		return nil, fmt.Errorf("failed to store email mapping: %v", err)
	}

	// Store password hash
	passwordKey := fmt.Sprintf("user:password:%s", user.ID)
...
// GetPasswordHash retrieves password hash for user
func (s *UserService) GetPasswordHash(userID string) (string, error) {
	passwordKey := fmt.Sprintf("user:password:%s", userID)
	hash, err := s.redisClient.Get(s.ctx, passwordKey).Result()
	if err != nil {
		if err == redis.Nil {
...
func (s *UserService) StoreRefreshToken(userID, refreshToken string) error {
	...
	ttl := time.Duration(s.cfg.JWTRefreshTTL) * time.Second
	err := s.redisClient.Set(s.ctx, refreshKey, refreshToken, ttl).Err()
	if err != nil {
...
func (s *UserService) GetRefreshToken(userID string) (string, error) {
	refreshKey := fmt.Sprintf("user:refresh:%s", userID)
	token, err := s.redisClient.Get(s.ctx, refreshKey).Result()
	if err != nil {
		if err == redis.Nil {
...
// DeleteRefreshToken deletes refresh token for user
func (s *UserService) DeleteRefreshToken(userID string) error {
	refreshKey := fmt.Sprintf("user:refresh:%s", userID)
	err := s.redisClient.Del(s.ctx, refreshKey).Err()
	if err != nil {
...
func (s *UserService) List(offset, limit int) ([]*models.User, error) {
	...
	pattern := "user:*"
	iter := s.redisClient.Scan(s.ctx, 0, pattern, int64(limit)).Iterator()
